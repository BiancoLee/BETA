    ...
    public class AftereOpenTreasury : IAftereOpenTreasury
    {
                ...
                switch (firstAccessType)
                {
                    case "P":
                        sealItem = _ITEM_SEAL
                            ...
                            .OrderByDescending(x => x.LAST_UPDATE_DT)
                            .AsEnumerable()
                            .Select(x => new SelectOption()
                            {
                                Value = x.ITEM_ID,
        ...
        public List<AfterOpenTreasurySearchDetailViewModel> GetSearchDetail(AfterOpenTreasurySearchViewModel searchData)
        {
            ...
            using (TreasuryDBEntities db = new TreasuryDBEntities())
            {
                ...
                var _TREA_ITEM_THREE = db.TREA_ITEM.AsNoTracking().Where(x => x.ITEM_OP_TYPE == "3").Select(x => x.ITEM_ID).ToList();
                ...
                if (_TREA_OPEN_REC.OPEN_TREA_TYPE == "1")
                {
                    ...
                    var _TREA_APLY_TEMP = db.TREA_APLY_TEMP
                        .Where(x => !_TREA_ITEM_THREE.Contains(x.ITEM_ID))
                ...
                switch (_TREA_ITEM.ITEM_OP_TYPE)
                {
                    ...
                    case "2":
                        if(_OTHER_ITEM_APLY != null)
                        {
                            ...
                            if (!_TREA_APLY_REC.ACCESS_TYPE.IsNullOrWhiteSpace())
                            {
                                accessType = _TREA_APLY_REC.ACCESS_TYPE;
                            }
                            else
                            {
                                accessType = _TREA_APLY_REC.ACTUAL_ACCESS_TYPE;
                            }
