    ...
    public class ItemChargeUnit : Common, IItemChargeUnit
    {
        ...
        public Tuple<List<SelectOption>, List<SelectOption>> DialogSelectedChange(string Charge_Dept, string Charge_Sect = null)
        {
            ...
            using (DB_INTRAEntities dbIntra = new DB_INTRAEntities())
            {
                ...
                _sectList.Add(Charge_Dept);

                dCharge_Uid = _V_EMPLY2
                    .Where(x => x.DPT_CD == Charge_Sect, Charge_Sect != "All" && !Charge_Sect.IsNullOrWhiteSpace())
                    .Where(x => _sectList.Contains(x.DPT_CD), _sectList.Count > 0 && Charge_Dept != "All" && Charge_Sect != " ")
                    .Where(x => x.DPT_CD == Charge_Dept, Charge_Dept != "All" && Charge_Sect == " ")
                    .AsEnumerable()
                    .Select(x => new SelectOption()
                    {
                        Value = x.USR_ID?.Trim(),
                ...
                //    .Where(x => _sectList.Contains(x.DPT_CD), _sectList.Count > 0 && Charge_Dept != "All")
                //    //.Where(x => x.DPT_CD == Charge_Dept)
        ...
        public Tuple<List<SelectOption>, List<SelectOption>> FirstDropDown()
        {
            ...
            {
                var _VW_OA_DEPT = dbIntra.VW_OA_DEPT.AsNoTracking();
                var _Dpt_types = new List<string>() {"03","02"};
