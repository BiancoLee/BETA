...
{
    public class AlreadyConfirmedSearch : Common, IAlreadyConfirmedSearch
    {
        ...
        public BaseUserInfoModel GetUserInfo(string cUserID)
        {
            ...
            {
                var _emply = dbINTRA.V_EMPLY2.AsNoTracking().FirstOrDefault(x => x.USR_ID != null && x.USR_ID == cUserID);
                if (_emply != null)
                {
                    user.EMP_ID = cUserID;
