...
,@APPR_DT
        ...
        **/
        ...
        public List<SelectOption> selectCodeRole(string ROLE_AUTH_TYPE = null)
        {
            using (dbTreasuryEntities context = new dbTreasuryEntities())
            {
                var result = context.CODE_ROLE
                    .Where(x => x.ROLE_AUTH_TYPE == ROLE_AUTH_TYPE, !string.IsNullOrWhiteSpace(ROLE_AUTH_TYPE));
                var items = new List<SelectOption>() { new SelectOption() { Value = " ", Text = "請選擇" } };
                items.AddRange(
                    result.Select(x => new SelectOption()
                    {
                        Text = x.ROLE_NAME,
                        Value = x.ROLE_ID
                    }));
        ...
        public String RoleLogContent(CODE_ROLE codeRole)
        {
            ...
            content += StringUtil.toString(codeRole.FREEZE_UID) + "|";
            content += codeRole.FREEZE_DT == null ? "|" : codeRole.FREEZE_DT + "|";
            content += codeRole.ROLE_AUTH_TYPE == null ? "|" : codeRole.ROLE_AUTH_TYPE + "|";  
