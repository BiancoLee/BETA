    ...
    public class TREASURY_REGISTER : ReportTreasuryRegister
    {
        ...
        private IEnumerable<Report_Treasury_Register> GetReportModel(IEnumerable<TREA_APLY_REC> data, List<TREA_ITEM> _Item_Desc, List<SYS_CODE> _Access_Type, List<BeforeOpenTreasurySeal> _Seal_Desc, List<V_EMPLY2> _Confirm)
        {
            return data.Select(x => new Report_Treasury_Register()
            {
                ...
                ACCESS_REASON = _Item_Desc.FirstOrDefault(y => y.ITEM_ID == x.ITEM_ID)?.ITEM_OP_TYPE != "3" ? x.ACCESS_REASON : "",   //入庫原因
                ACCESS_NAME = !x.CONFIRM_UID.IsNullOrWhiteSpace() ? _Confirm.FirstOrDefault(y => y.USR_ID == x.CONFIRM_UID)?.EMP_NAME?.Trim() : null,    //入庫人員
                //ACCESS_NAME = !x.CONFIRM_UID.IsNullOrWhiteSpace() ? x.CONFIRM_UID + "-" + _Confirm.FirstOrDef1ault(y => y.USR_ID == x.CONFIRM_UID)?.EMP_NAME : null,    //入庫人員
                ACTUAL_ACCESS_TYPE = string.IsNullOrEmpty(x.ACTUAL_ACCESS_TYPE) ? null : x.ACTUAL_ACCESS_TYPE != x.ACCESS_TYPE ? _Access_Type.FirstOrDefault(y => y.CODE == x.ACTUAL_ACCESS_TYPE)?.CODE_VALUE : null,   //實際作業別
                //ACTUAL_ACCESS_TYPE = _Access_Type.FirstOrDefault(y => y.CODE == x.ACTUAL_ACCESS_TYPE)?.CODE_VALUE,    //實際作業別
                ACTUAL_ACCESS_NAME = string.IsNullOrEmpty(x.ACTUAL_ACCESS_UID) ? null : _Confirm.FirstOrDefault(y => y.USR_ID == x.ACTUAL_ACCESS_UID)?.EMP_NAME?.Trim()   //實際入庫人員
